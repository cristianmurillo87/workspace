		...
		if (task) {
			try {
				if (task.taskType !== 'reportGeneration') {
					...
					// Run report generation task only when all preceding tasks are completed
					const allCompleted = workflow?.tasks?.every((tsk) =>
						[TaskStatus.Completed, TaskStatus.Failed, TaskStatus.Skipped].includes(tsk.status),
					);

					if (allCompleted) await taskRunner.run(task);
				}
			} catch (error) {
